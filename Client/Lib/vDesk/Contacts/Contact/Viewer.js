...
vDesk.Contacts.Contact.Viewer = function Viewer(Contact) {
    ...
    CountryRow.textContent = (vDesk.Locale.Countries.find(Country => Country.Code === Contact.Country.Code) || {Name: ""}).Name;
    ...
    CompanyRow.textContent = `${vDesk.Locale["Contacts"]["Company"]}: ${(vDesk.Contacts.Companies.find(
        Company => Company.ID === Contact.Company.ID) || {Name: ""}).Name}`;
