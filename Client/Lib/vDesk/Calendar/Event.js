...
vDesk.Calendar.Event = function Event(
    ...
    this.Extends(vDesk.Controls.HeaderedResizableBox);
    ...
    this.Parent.Content.textContent = Content;
    ...
    this.Movable = AccessControlList.Write && !FullTime;
    this.Resizable = this.Movable;
    this.ResizableLeft = false;
    this.ResizableRight = false;
    this.SetTooltip();
    this.Control.addEventListener("resize", OnResizeAbove, true);
...
vDesk.Calendar.Event.FromDataView = function(DataView) {
    ...
    return new vDesk.Calendar.Event(
        DataView.ID || null,
        DataView.Owner.ID !== undefined
        ? vDesk.Security.Users.find(User => User.ID === DataView.Owner.ID)
        : vDesk.Security.User.FromDataView(DataView.Owner),
        DataView.Start || null,
        DataView.End || null,
