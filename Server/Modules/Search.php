...
final class Search extends Module {
    ...
    public static function Search(string $Value = null, array $Filters = null): Results {
        ...
        foreach($Filters ?? Command::$Parameters["Filters"] as $Filter) {
            ...
            } catch(\vDesk\Modules\UnknownModuleException $ex) {
                Log::Warn(__METHOD__, "Attempting to perform search operation on unknown Module {$Filter["Module"]}.");
                continue;
            } catch(\Exception $ex) {
                Log::Warn(__METHOD__, $ex->getMessage());
                continue;
            }
        }
