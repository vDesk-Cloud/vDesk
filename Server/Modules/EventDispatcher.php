...
final class EventDispatcher extends Module implements IModule {
    ...
    public static function Schedule(): void {
        ...
        if(self::$Events->Count > 0) {
            ...
            //if(Settings::$Local["EventDispatcher"]["Events"] !== null)
            ...
            foreach(
                Expression::Select("File")
                          ...
                          ->Where(
                              [
                                  "Parent"    => Settings::$Local["EventDispatcher"]["Events"],
                                  "Extension" => "php",
    ...
    /**
     * Gets a EventProvider that continuously provides event-messages.
     *
     * @return \vDesk\Events\EventProvider The EventProvider
     */
    public static function GetEvents(): EventProvider {
        return new EventProvider(Settings::$Remote["EventDispatcher"]["Interval"]);
    }
        ...
        if($Package instanceof Events\IPackage) {
            ...
            foreach($Package::Events as $Event => $File) {
                ...
                \vDesk\Modules::Archive()::Upload(
                    new Element(Settings::$Local["EventDispatcher"]["Events"]),
        ...
        if($Package instanceof Events\IPackage) {
            foreach(
                Expression::Select("ID")
                          ...
                          ->Where([
                              "Parent" => Settings::$Local["EventDispatcher"]["Events"],
                              "Name"   => ["IN" => \array_map(fn($File): string => Path::GetFileName($File, false), \array_values($Package::Events))]
                          ])
                as $Listener
            ) {
