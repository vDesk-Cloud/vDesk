...
<head>
    ...
    <link rel="stylesheet" href="<?= Functions::Stylesheet("vDesk") ?>">
...
<article class="Page">
        ...
        <nav>
            <?php foreach($Pages as $Page): ?>
                <?php if($Page->Name !== "Index"): ?>
                    <a class="<?= $Page->Name === "Tutorials" ? "Current" : "" ?>"
        ...
        <nav>
            <?php foreach($Tutorials as $Tutorial): ?>
                <?php if($Tutorial->Name !== "Index"): ?>
                    <a class="<?= $Tutorial->Name === $Current->Name ? "Current" : "" ?>"
    ...
    <section>
        <?= Functions::Template("Documentation/Tutorials/{$Current->Name}", ["Current" => $Current, "Pages" => $Pages, "Tutorials" => $Tutorials ?? []]) ?>
    </section>
    <footer>
        Copyright (C) 2020 Kerry Holz
    </footer>
</article>
</body>
</html>