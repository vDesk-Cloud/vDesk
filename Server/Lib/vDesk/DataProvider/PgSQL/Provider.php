...
class Provider extends \vDesk\DataProvider\AnsiSQL\Provider {
    ...
    /**
     * The last autogenerated value caused by the execution of an INSERT statement.
     *
     * @var int
     */
    ...
    public function Sanitize(mixed $Value): string|int {
        if($Value instanceof IManagedModel && $Value->ID() === null) {
            return "DEFAULT";
        }
        return match (Type::Of($Value)) {
            Type::String => $Value === "DEFAULT" ? $Value : parent::Sanitize($Value),
            default => parent::Sanitize($Value)
        };
    }
