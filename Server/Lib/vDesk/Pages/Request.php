...
class Request extends Command {
    ...
    public static function Parse(): self {
        ...
        static::$Ticket      = Input::ParseCommand("Ticket") ?? $_COOKIE["Ticket"] ?? null;
        ...
        foreach(Settings::$Local["Routes"] as $Route => $Definition) {
            ...
            if(
                ...
                static::$Parameters = new Parameters(\array_combine($Parameters, $Values));
                static::$Ticket     = Input::ParseCommand("Ticket") ?? $_COOKIE["Ticket"] ?? null;
                return new static();
            }
        }

        static::$Parameters = new Parameters();
