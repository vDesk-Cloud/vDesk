...
class Client {
     ...
     *
     * @param \vDesk\Package $Package The Package to add the files of.
     */
    ...
    public function ToHTML(bool $Inline = false): string {
        $Design  = \implode(\PHP_EOL, \array_map(fn(string $Stylesheet): string => "        <link rel=\"stylesheet\" type=\"text/css\" href=\"./Design{$Stylesheet}\">", $this->Design));
        $Lib     = \implode(\PHP_EOL, \array_map(fn(string $Script): string => "        <script src=\"./Lib{$Script}\"></script>", $this->Lib));
        $Modules = \implode(\PHP_EOL, \array_map(fn(string $Module): string => "        <script src=\"./Modules{$Module}\"></script>", $this->Modules));
