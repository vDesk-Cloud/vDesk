...
class Contacts extends Collection implements IDataView {
     ...
     *
     * @param iterable                  $Elements Initializes the Contacts with the specified set of elements.
     * @param null|\vDesk\Security\User $User     Fills the collection according to the user.
     * @param null|int                  $From     Specifies a start index within a range of contacts.
     * @param null|int                  $To       Specifies a end index within a range of contacts.
     */
    ...
    public static function FromDataView(mixed $DataView): IDataView {
        return new static(
            (static function() use ($DataView) {
                foreach($DataView as $Contact) {
                    yield Contact::FromDataView($Contact);
                }
            })()
        );
    }
    ...
    private function Fill(int $From = null, int $To = null): void {
        ...
        foreach($Expression as $Row) {
            ...
            if($Contact->AccessControlList->Read) {
                ...
                $Contact->City        = $Row["City"];
                $Contact->Country     = new Country($Row["Country"]);
                $Contact->Options     = new Options([], $Contact);
