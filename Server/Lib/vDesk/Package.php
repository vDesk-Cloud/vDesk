...
abstract class Package implements IDataView {
    ...
    /**
     * Composes the files and folders of Package into a specified Phar archive.
     *
        ...
        foreach(static::Files as $Target => $Types) {
            foreach($Types as $Type => $Resources) {
                if(\is_array($Resources)) {
                    ...
                    foreach(self::Resolve($Target, $Type) as $Name => $FileSystemInfo) {
                    ...
                    Log::Debug(__METHOD__, $Resources);
                    foreach(self::Resolve($Target, $Resources) as $Name => $FileSystemInfo) {
                        if($FileSystemInfo instanceof FileInfo) {
                            Log::Debug(__METHOD__, $Name);
                            Log::Debug(__METHOD__, $FileSystemInfo->FullName);
    ...
    /**
     * Undeploys the files and folders of the Phar archive of the Package to a specified path.
     */
     ...
     * @inheritDoc
     */
    final public static function FromDataView($DataView): IDataView {
        throw new \RuntimeException("Package::FromDataView() is not supported!");
    }
